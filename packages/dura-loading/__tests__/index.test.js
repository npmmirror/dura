...
describe('测试loading 插件', function () {
    it('测试loading 插件,启用loading', function (done) {
        ...
        setTimeout(function () { return expect(getState().loading.user.onAsyncChangeName).toEqual(true); }, 300);
        setTimeout(function () {
    ...
    it('测试loading 插件,不启用loading', function (done) {
        ...
        setTimeout(function () { return expect(getState().loading.user.onAsyncChangeName).toEqual(false); }, 300);
        setTimeout(function () {
            expect(getState().user.name).toEqual('张三');
            expect(getState().loading.user.onAsyncChangeName).toEqual(false);
            done();
