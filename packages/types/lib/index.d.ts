...
export declare type ReducersMapOfStoreSlice<S> = JsonObject<Reducer<S>>;
export declare type EffectsMapOfStore = JsonObject<EffectsMapOfStoreSlice>;
export declare type ReducersMapOfStore<S> = JsonObject<ReducersMapOfStoreSlice<S>>;
export declare type StoreSliceMap<N extends string, S extends JsonObject, R extends ReducersMapOfStoreSlice<S>, E extends EffectsMapOfStoreSlice> = JsonObject<StoreSlice<N, S, R, E>>;
export interface StoreSlice<N extends string, S extends JsonObject, R extends ReducersMapOfStoreSlice<S>, E extends EffectsMapOfStoreSlice> {
    namespace: N;
    state: S;
    reducers?: R;
    effects?: E;
}
export declare type ExtractStateByStoreUnion<T> = T extends StoreSlice<infer N, infer S, infer R, infer E> ? {
    [K in N]: {
        [SK in keyof S]: S[SK];
    };
} : never;
export declare type ExtractAction<T> = ExtractActionByReducer<T> & ExtractActionByEffect<T>;
export declare type ExtractActionByReducer<T> = T extends StoreSlice<infer N, infer S, infer R, infer E> ? {
    [K in N]: {
...
export interface UseStoreSliceFn<GS, GA> {
    <N extends string, S extends JsonObject, R extends ReducersMapOfStoreSlice<S>, E extends EffectsMapOfStoreSlice, STORES extends StoreSlice<N, S, R, E>[], NGA = UnionToIntersection<ExtractAction<STORES[number]>>, NGS = UnionToIntersection<ExtractStateByStoreUnion<STORES[number]>>>(...stores: STORES): CreateStoreReturn<GS & NGS, GA & NGA>;
}
