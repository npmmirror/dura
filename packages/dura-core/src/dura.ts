...
export default class Dura {
    ...
    private mergeReducers() {
        return combineReducers(this.gModels.map(this.extractReducer.bind(this)).reduce((prev, next) => ({...prev, ...next}), {}))
    }
    ...
    private delModel(name: string) {
        const exits = this.gModels.some((model: IModel) => model.name === name);
        if (!exits) {
            throw new Error("[mode.name]there is no")
        }
        this.gModels = this.gModels.filter((model: IModel) => model.name !== name)
    }
    ...
    public createDuraStore(): DuraStore {
        ...
        this.duraStore = {
            ...
            unModels: (...names: Array<string>) => {
                names.forEach(this.delModel.bind(this))
                this.duraStore.replaceReducer(this.mergeReducers())
            }
        };
