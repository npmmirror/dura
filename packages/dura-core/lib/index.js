...
function extractReducers(name, model) {
  ...
  var nextReducer = reducerKeys
    .map(function(reducerName) {
      ...
      return (
        (_a = {}),
        (_a[name + "/" + reducerName] = function(state, action) {
          return reducers[reducerName](action.payload, action.meta)(state);
        }),
        _a
      );
    })
...
function mergeModel(config) {
  var _a = config.initialModel,
    initialModel = _a === void 0 ? {} : _a,
    _b = config.plugins,
...
function wrapRootModel(rootModel, plugin) {
  var wrapModelPlugins = plugin.filter(function(p) {
    return p.onWrapModel;
  });

  //包装已有的model
  return Object.keys(rootModel)
    .map(function(name) {
      return wrapModel(wrapModelPlugins, name, rootModel[name]);
    })
    .reduce(function(prev, next) {
      return __assign({}, prev, next);
    }, {});
...
function create(config) {
  ...
  var rootReducers = Object.keys(nextRootModel)
    .map(function(name) {
      return extractReducers(name, nextRootModel[name]);
    })
    .reduce(function(prev, next) {
      return __assign({}, prev, next);
    }, {});
  var middlewares = plugins
    .filter(function(p) {
      return p.onCreateMiddleware;
    })
    .map(function(p) {
      return p.onCreateMiddleware(nextRootModel);
    });
  //store增强器
  ...
  plugins
    .filter(function(p) {
      return p.onStoreCreated;
    })
    .forEach(function(p) {
      return p.onStoreCreated(reduxStore, nextRootModel);
    });
...
function createModelReducerRunner(name, model, dispatch) {
  var _a;
  var _b = model.reducers,
    reducers = _b === void 0 ? {} : _b;
  var reducerKeys = Object.keys(reducers);
...
function createReducerRunner(models, dispatch) {
  ...
  return Object.keys(models)
    .map(function(name) {
      return createModelReducerRunner(name, models[name], dispatch);
    })
    .reduce(merge, {});
}
