...
因为 dura 本身是基于 redux 生态来构建的，所以事实上 redux 整个生态的插件基本上大多数都是围绕着 reducer、middleware 或者 覆写一部分 redux 的函数 来进行的， 所以我在思考：本质上 dura 究竟需不需要单独的提出插件的概念？当然目前的版本是存在的，很有可能在下个**主版本**中会将插件的概念移除，转而采用一些辅助性的工具函数来实现插件的功能。
...
- **【2019.02.09】** 1.0.3 [@dura/plus](https://github.com/CN-YUANYU/dura/tree/master/packages/dura-plus) 中移除了 selector 方案 [f249a6a](https://github.com/CN-YUANYU/dura/commit/f249a6ac6165954808199fe047cd0e93e48d16c8)
...
- **validation** 机制（摸索阶段），虽然说我们鼓励扁平的数据格式，但是免不了总会存在数据嵌套，这里主要需要摸索在数据嵌套，尤其是列表的大背景下，如果使用 **验证机制** 更为友好
- **脚手架工程**
